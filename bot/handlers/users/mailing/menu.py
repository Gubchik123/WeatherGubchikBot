...
variants = ("управління розсилкою",
            "управление рассылкой", "mailing management")
...
async def mailing_menu(message: types.Message):
    ...
    await message.answer(
        TEXT().mailing_info_message(
            data["time_int"], mute, data["time"], data["city"])
    )
    await message.answer(
        TEXT().what_do_you_want_to_do_with_mailing_message(),
        reply_markup=_get_mailing_menu_keyboard_with_(mute_btn_text)
    )
