...
def _parse_catalog_items_from_(text: str, lang_code: str) -> BeautifulSoup:
    ...
    catalog_items = catalog_lists[-1].find_all(
...
def _extract_city_data(
    catalog_item: BeautifulSoup, user_input: str, cities: dict
) -> Union[str, None]:
    """Extracts city title and url from the given catalog item.
    Fills the given cities dict or return exact city url."""
    link = catalog_item.find("a")
    city_title = link.text.strip().lower()
